...
export function CelebrationOverlay({ data, onDismiss }: { data: Celebration; onDismiss: () => void }) {
  return (
    <div
      onClick={onDismiss}
    ...
    >
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 330, background: T.solid, borderRadius: 30, padding: '30px 26px 24px', textAlign: 'center', boxShadow: '0 26px 60px rgba(0,0,0,.32)', animation: 'pep-pop .45s ease' }}>
        ...
        <button
          onClick={onDismiss}
          className="pressable"
          style={{ width: '100%', background: T.accent, color: T.ink, border: 'none', borderRadius: T.r.pill, padding: 15, fontFamily: T.display, fontWeight: 600, fontSize: 17, cursor: 'pointer', ['--press-y' as string]: '2px' }}
        >
          Nice!
        </button>
      </div>
